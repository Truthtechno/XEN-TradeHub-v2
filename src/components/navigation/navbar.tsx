...
const Navbar = () => {
    if (!clerkEnabled) {
        return <NavbarContent isSignedIn={false} />;
    }

    return <NavbarWithClerk />;
};

const NavbarWithClerk = () => {
    const { user } = useClerk();
    return <NavbarContent isSignedIn={Boolean(user)} />;
};

const NavbarContent = ({ isSignedIn }: { isSignedIn: boolean }) => {
    ...
    return (
        <header className={cn(
            "sticky top-0 inset-x-0 h-14 w-full border-b border-transparent z-[99999] select-none transition-colors duration-200",
            scroll ? "border-white/10 bg-black/85 backdrop-blur-md" : "bg-transparent"
        )}>
                    ...
                    <div className="flex items-center space-x-12">
                        ...
                        <NavigationMenu className="hidden lg:flex">
                            <NavigationMenuList>
                                ...
                                {SHOW_LEGACY_NAV_LINKS && NAV_LINKS.map((link) => (
                                    <NavigationMenuItem key={link.title}>
                                        {link.menu ? (
                                            <>
                    ...
                    <div className="hidden lg:flex items-center">
                        {isSignedIn ? (
                            ...
                            <div className="flex items-center gap-x-4">
                                <Link href="/admin/login" className={buttonVariants({ size: "sm", variant: "ghost" })}>
                                    Sign In
                                </Link>
                                <Link href="/#trade-with-us" className={buttonVariants({ size: "sm", })}>
                                    Get Started
