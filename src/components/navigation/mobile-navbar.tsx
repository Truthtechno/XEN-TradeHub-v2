...
const SHOW_LEGACY_NAV_LINKS = false;
const HOME_NAV_LINKS = [
    { title: "Markets", href: "/#markets" },
    { title: "Why Us", href: "/#why-choose-us" },
    { title: "Trade With Us", href: "/#trade-with-us" },
    { title: "Contact", href: "/#contact" },
] as const;

const MobileNavbar = () => {
    if (!clerkEnabled) {
        return <MobileNavbarContent isSignedIn={false} />;
    }

    return <MobileNavbarWithAuth />;
};

const MobileNavbarWithAuth = () => {
    const { isSignedIn } = useAuth();
    return <MobileNavbarContent isSignedIn={Boolean(isSignedIn)} />;
};

const MobileNavbarContent = ({ isSignedIn }: { isSignedIn: boolean }) => {
    ...
    return (
        <div className="flex lg:hidden items-center justify-end">
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
                ...
                <SheetContent className="w-[88vw] max-w-sm border-l border-border/40 bg-background/95 backdrop-blur-md">
                        ...
                        <div className="flex items-center justify-evenly w-full space-x-2">
                            {isSignedIn ? (
                                ...
                                <>
                                    ...
                                    <Link href="/#trade-with-us" className={buttonVariants({ className: "w-full" })}>
                                        Get Started
                                    </Link>
                                </>
                            )}
                            ...
                            <div className="w-full">
                                {HOME_NAV_LINKS.map((link) => (
                                    <Link
                                        ...
                                        href={link.href}
                                        onClick={handleClose}
                                        className="flex items-center w-full py-3.5 px-3 rounded-md font-medium text-muted-foreground hover:text-foreground hover:bg-accent/60 transition-colors"
                                    >
                                        <span>{link.title}</span>
                                    </Link>
                                ))}
                            </div>
                            {SHOW_LEGACY_NAV_LINKS && (
